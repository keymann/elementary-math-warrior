...
export const BALANCE = {
   ...
   *
   * 시간 배수 위에 곱해지므로 후반 배수가 급격해진다 — 용암 지대(6~8분) 기준
   * 체력 (1 + 480/200) × 1.55 ≈ 5.3배, 속도 ×1.23.
   *
   * 지형 이동 배수와 **곱해진다**는 점에 주의. 사막의 적 감속(0.95)처럼 1보다 작은
   * 값이 있으면 라운드 계단이 상쇄될 수 있다. 실제로 사막 감속이 0.88이던 때
   * 2라운드 적이 숲보다 느렸다.
   */
  round: [
    { hp: 1, speed: 1, label: '숲' },
    { hp: 1.16, speed: 1.1, label: '사막' },
    { hp: 1.34, speed: 1.16, label: '현무암' },
    { hp: 1.55, speed: 1.23, label: '용암' },
    { hp: 1.82, speed: 1.3, label: '하늘' },
  ...
  biome: {
    ...
    /**
     * 모래에 발이 빠진다. 적도 함께 느려져 억울하지 않게 한다.
     *
     * 0.88 이었을 때 라운드 가속(×1.12)과 곱해도 0.99 라서 **사막의 적이 숲보다
     * 느렸다.** 지형 설계와 라운드 난이도 계단이 서로 상쇄돼 2라운드가 오히려
     * 쉬워지는 구간이 됐다. 플레이어 감속은 유지하되 적 감속만 완화한다.
     */
    desert: { player: 0.9, enemy: 0.95, accel: 26, gem: 1, label: '모래에 발이 푹푹 빠진다' },
